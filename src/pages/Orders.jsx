                ...
                <div key={o.id} className="rounded-[28px] border border-line bg-deep overflow-hidden">
                  ...
                  <button onClick={() => setOpenId(open ? null : o.id)} className="w-full flex flex-wrap items-center gap-3 px-5 py-4 text-left hover:bg-glow/5 transition-colors">
                    ...
                    <span className="flex-1 min-w-[140px]">
                      ...
                      <span className="block text-[11px] text-mist">Placed {fmtDate(o.createdAt)}</span>
                    </span>
                    <span className={`text-[10px] font-black uppercase tracking-widest rounded-full px-3 py-1 ${(o.status || 'confirmed') === 'confirmed' ? 'bg-glow/10 text-glow border border-glow/30' : 'bg-mint/10 text-mint border border-mint/30'}`}>
                      {o.status || 'Confirmed'}
                    </span>
                    ...
                    <div className="border-t border-line px-5 py-5 space-y-5">
                        ...
                        {STATUS_STEPS.map((s, i) => (
                          <React.Fragment key={s.id}>
                            <div className="flex flex-col items-center gap-1.5 flex-1 min-w-0">
                              <span className={`w-3 h-3 rounded-full ${i <= statusIdx ? 'bg-glow' : 'bg-line'}`} />
                              <span className={`text-[9px] uppercase tracking-wide font-bold ${i <= statusIdx ? 'text-glow' : 'text-mist'}`}>{s.label}</span>
                            </div>
                      ...
                      <div className="grid sm:grid-cols-2 gap-4 text-xs">
                        <div className="rounded-2xl border border-line bg-panel/50 p-4">
                          <p className="text-[10px] uppercase tracking-widest text-mist font-bold mb-2">Deliver to</p>
                        ...
                        <div className="rounded-2xl border border-line bg-panel/50 p-4">
                          <p className="text-[10px] uppercase tracking-widest text-mist font-bold mb-2">Payment</p>
